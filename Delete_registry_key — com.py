...
if selected_language == "Russian":
    def check_version(current_version):
        ...
        try:
            ...
            print(Fore.RED + "Проверка невозможна без подключения к интернету")
            play_sound(sound_error)
            return
        ...
        try:
            ...
            print(Fore.RED + "Сервера недоступны. Пожалуйста, проверьте наличие обновлений позже")
            play_sound(sound_error)
            return
        ...
        if parse(online_version) > parse(current_version):
            print(
                Fore.YELLOW + f"Доступно обновление {online_version}! Ваша версия {current_version}! Для получения инструкций по установке обновления, пожалуйста, скачайте установщик с https://github.com/Romuilesik/WinArchiveInstaller или с https://sites.google.com/view/romuilesik-winarchive/main/download?authuser=0, запустите установщик и начните установку")

            url_notes = "https://pastebin.com/raw/VQySEM02"
            try:
                response_notes = requests.get(url_notes)
                response_notes.raise_for_status()
            except (requests.RequestException, ValueError):
                print(Fore.RED + "Не удалось получить примечания к выпуску. Пожалуйста, попробуйте позже")
                return

            release_notes = response_notes.text.strip()
            if release_notes:
                print(Fore.YELLOW + "\nПримечания к выпуску:")
                print(release_notes)
                print("")
            else:
                print("Примечания к выпуску отсутствуют.")
                ...
                if confirm == "1":
                    def delete_default_icon(extension):
                        try:
                            ...
                            print(Fore.YELLOW + f"""Если у вас не получается удалить ключ реестра попробуйте сделать это вручную по этой инстуркции:
...
if selected_language == "English":
    ...
    def check_version(current_version):
        ...
        try:
            ...
            print(Fore.RED + "Check is not possible without an internet connection")
            play_sound(sound_error)
            return
        ...
        try:
            ...
            print(Fore.RED + "Servers are unavailable. Please check for updates later")
            play_sound(sound_error)
            return
        ...
        if parse(online_version) > parse(current_version):
            print(
                Fore.YELLOW + f"Update {online_version} available! Your version {current_version}! To get installation instructions, please download the installer from https://github.com/Romuilesik/WinArchiveInstaller or from https://sites.google.com/view/romuilesik-winarchive/main/download?authuser=0, run the installer, and begin installation")
            ...
            try:
                ...
                print(Fore.RED + "Failed to retrieve release notes. Please try again later")
                return
            ...
            if release_notes:
                ...
                print("Release notes are not available.")

            play_sound(sound_warning)
        elif parse(online_version) < parse(current_version):
            print(Fore.YELLOW + "You are participating in the beta testing of the program!")
            play_sound(sound_warning)
        else:
...
if selected_language == "Ukrainian":
    ...
    def check_version(current_version):
        ...
        try:
            ...
            print(Fore.RED + "Перевірка неможлива без підключення до Інтернету")
            play_sound(sound_error)
            return
        ...
        try:
            ...
            print(Fore.RED + "Сервери недоступні. Будь ласка, спробуйте перевірити оновлення пізніше")
            play_sound(sound_error)
            return
        ...
        if parse(online_version) > parse(current_version):
            print(
                Fore.YELLOW + f"Оновлення {online_version} доступне! Ваша версія {current_version}! Щоб отримати інструкції щодо установки, будь ласка, завантажте встановлювач з https://github.com/Romuilesik/WinArchiveInstaller або з https://sites.google.com/view/romuilesik-winarchive/main/download?authuser=0, запустіть встановлювач і розпочніть установку")
            ...
            try:
                ...
                print(Fore.RED + "Не вдалося отримати примітки до релізу. Будь ласка, спробуйте ще раз пізніше")
                return
            ...
            if release_notes:
                ...
                print("Примітки до релізу відсутні.")
...
: """)
                ...
                if language in languages:
                    ...
                    print(Fore.RED + "Некоректний вибір мови")
                    play_sound(sound_error)
